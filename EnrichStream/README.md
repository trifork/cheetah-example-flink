...
When developing your job you can run/debug it like any other Java application by running the `main` method in EnrichStreamJob.

### Create intellij run profile for EnrichStreamJob.java with parameters
...
This project contains a sample Unit test in `src/test/java/cheetah/example/job/EnrichStreamMapperTest.java`, which utilizes JUnit5.
...
```sh
docker compose up enrichstream-job enrichstream-job-taskmanager --build
```

Similarly, you can run just the component test through docker compose using:
```sh
docker compose up enrichstream-test --build
```
