...
You need to create the source topics which the flink job reads from. This is done by setting the `INITIAL_KAFKA_TOPICS` to `SerializationIssueInputTopic SerializationIssueOutputTopic` before running the `kafka-setup` container in `cheetah-development-infrastructure`:
```powershell
$env:INITIAL_KAFKA_TOPICS="SerializationIssueInputTopic SerializationIssueOutputTopic"; docker compose up kafka-setup -d
```
...
When developing your job you can run/debug it like any other Java application by running the `main` method in SerializationIssueJob.
...
1. Navigate to the file `SerializationIssueJob.java`, under the `src/main/java/com/cheetah/example/job/` folder.
...
1. Create Intellij run profile for SerializationIssueJob.java with parameters, by right-clicking the file `SerializationIssueJob` and select `Modify Run Configuration...`.
1. Enter the following program arguments:

  ```
  --kafka-bootstrap-servers localhost:9092
  --input-kafka-topic SerializationIssueInputTopic
  --output-kafka-topic SerializationIssueOutputTopic
  --kafka-group-id SerializationIssue-group-id
  ```
  ...
  ```
1. Notice how the job is configured to consume events from topic `SerializationIssueInputTopic` and output to `SerializationIssueOutputTopic`
...
This project contains a sample Unit test in `src/test/java/com/cheetah/example/job/SerializationIssueMapperTest.java`, which utilizes JUnit5.
...
```sh
docker compose up serializationissue-jobmanager serializationissue-taskmanager --build
```

Similarly, you can run just the component test through docker compose using:
```sh
docker compose up serializationissue-test --build
```

If doing so, make sure to run your job locally from Intellij before starting the component test.

The component test is producing a single message to `SerializationIssueInputTopic`, and listening for any messages published to `SerializationIssueOutputTopic`. It expects the flink job to publish the same message, enriched with a new field, on `SerializationIssueOutputTopic`.
