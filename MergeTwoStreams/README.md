...
The streams will be joined based on the deviceId found in both input-streams.
...
When developing your job you can run/debug it like any other Java application by running the `main` method in MergeTwoStreamsJob.

### Create intellij run profile for MergeTwoStreamsJob.java with parameters

Program arguments:
```
--kafka-bootstrap-servers localhost:9092
--input-kafka-topic MergeTwoStreamsInputTopic
--output-kafka-topic MergeTwoStreamsOutputTopic
```
...
This project contains a sample Unit test in `src/test/java/cheetah/example/job/MergeTwoStreamsMapperTest.java`, which utilizes JUnit5.
...
```sh
docker compose up mergetwostreams-job mergetwostreams-job-taskmanager --build
```

Similarly, you can run just the component test through docker compose using:
```sh
docker compose up mergetwostreams-test --build
```
