...
You need to create the source topics which the flink job reads from. This is done by setting the `INITIAL_KAFKA_TOPICS` to `fepaInputTopic fepaOutputTopic` before running the `kafka-setup` container in `cheetah-development-infrastructure`:
```powershell
$env:INITIAL_KAFKA_TOPICS="fepaInputTopic fepaOutputTopic"; docker compose up kafka-setup -d
```
...
When developing your job you can run/debug it like any other Java application by running the `main` method in fepaJob.
...
1. Navigate to the file `fepaJob.java`, under the `src/main/java/fepa/job/` folder.
...
1. Create Intellij run profile for fepaJob.java with parameters, by right-clicking the file `fepaJob` and select `Modify Run Configuration...`.
1. Enter the following program arguments:
  ...
  ```
1. Notice how the job is configured to consume events from topic `fepaInputTopic` and output to `fepaOutputTopic`
...
This project contains a sample Unit test in `src/test/java/fepa/job/fepaMapperTest.java`, which utilizes JUnit5.
...
```sh
docker compose up fepa-jobmanager fepa-taskmanager --build
```

Similarly, you can run just the component test through docker compose using:
```sh
docker compose up fepa-test --build
```

If doing so, make sure to run your job locally from Intellij before starting the component test.

The component test is producing a single message to `fepaInputTopic`, and listening for any messages published to `fepaOutputTopic`. It expects the flink job to publish the same message, enriched with a new field, on `fepaOutputTopic`.
