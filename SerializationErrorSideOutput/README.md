...
You need to create the source topics which the flink job reads from. This is done by setting the `INITIAL_KAFKA_TOPICS` to `SerializationErrorSideOutputInputTopic SerializationErrorSideOutputOutputTopic` before running the `kafka-setup` container in `cheetah-development-infrastructure`:

```powershell
$env:INITIAL_KAFKA_TOPICS="SerializationErrorSideOutputInputTopic SerializationErrorSideOutputOutputTopic"; docker compose up kafka-setup -d
```
...
When developing your job you can run/debug it like any other Java application by running the `main` method in SerializationErrorSideOutputJob.
...
1. Navigate to the file `SerializationErrorSideOutputJob.java`, under the `src/main/java/cheetah/example/serializationerrorsideoutput/job/` folder.
...
1. Create Intellij run profile for SerializationErrorSideOutputJob.java with parameters, by right-clicking the file `SerializationErrorSideOutputJob` and select `Modify Run Configuration...`.
1. Enter the following program arguments:

  ``` bash
  --kafka-bootstrap-servers localhost:9092
  --input-kafka-topic SerializationErrorSideOutputInputTopic
  --output-kafka-topic SerializationErrorSideOutputOutputTopic
...
This project contains a sample Unit test in `src/test/java/cheetah/example/serializationerrorsideoutput/job/SerializationErrorSideOutputMapperTest.java`, which utilizes JUnit5.
...
```sh
docker compose up serializationerrorsideoutput-jobmanager serializationerrorsideoutput-taskmanager --build
```

Similarly, you can run just the component test through docker compose using:

```sh
docker compose up serializationerrorsideoutput-test --build
```

If doing so, make sure to run your job locally from Intellij before starting the component test.

The component test is producing a single message to `SerializationErrorSideOutputInputTopic`, and listening for any messages published to `SerializationErrorSideOutputOutputTopic`. It expects the flink job to publish the same message, enriched with a new field, on `SerializationErrorSideOutputOutputTopic`.
