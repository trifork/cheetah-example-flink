...
You need to create the source topics which the flink job reads from. This is done by setting the `INITIAL_KAFKA_TOPICS` to `ExternalLookupInputTopic` before running the `kafka-setup` container in `cheetah-development-infrastructure`:
```powershell
$env:INITIAL_KAFKA_TOPICS="ExternalLookupInputTopic"; docker compose up kafka-setup -d
```
...
When developing your job you can run/debug it like any other Java application by running the `main` method in ExternalLookupJob.

### Create intellij run profile for ExternalLookupJob.java with parameters
...
This project contains a sample Unit test in `src/test/java/Externallookup/job/ExternalLookupMapperTest.java`, which utilizes JUnit5.
...
```sh
docker compose up externallookup-job externallookup-job-taskmanager --build
```

Similarly, you can run just the component test through docker compose using:
```sh
docker compose up externallookup-test --build
```
